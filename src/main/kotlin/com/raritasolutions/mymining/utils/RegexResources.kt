...
val teacherRankNoDots = "(Доц|Проф|Асс|Асп|Стпр|Преп)".toRegex(RegexOption.IGNORE_CASE)
...
val roomMiningMuseumRegex = "($roomSearchingRegex\\s?)?Горн.*муз.*".toRegex()
val roomMiningMuseumWithRoomRegex = "Горн.*муз.*зал.*\\d{2,}".toRegex()
...
val teacherNoRankRegex = "([А-ЯЁ][а-яё]{2,}\\s?$teacherInitials|$vacancyRegex)".toRegex()
